...
REMOTE_REPO_DIR="${REMOTE_REPO_DIR:-/root/code/zagreb-bus-stops}"
REMOTE_DOCROOT="${REMOTE_DOCROOT:-/var/www/zagreb.lol/bus-stop-shade}"
CLONE_URL="${CLONE_URL:-git@github-personal:Poglavar/zagreb-bus-stops.git}"
BRANCH="main"
...
echo "Deploying zagreb-stanice — server pulls ${BRANCH} on ${REMOTE_HOST}"
...
if [ -n "${CF_ZONE_ID:-}" ] && [ -n "${CF_API_KEY:-}" ]; then
	echo "Purging Cloudflare cache for https://zagreb.lol/bus-stop-shade/"
	curl -s -X POST "https://api.cloudflare.com/client/v4/zones/${CF_ZONE_ID}/purge_cache" \
		-H "Authorization: Bearer ${CF_API_KEY}" \
		-H "Content-Type: application/json" \
		--data '{"prefixes":["https://zagreb.lol/bus-stop-shade/"]}' >/dev/null && echo "Cloudflare cache purged."
else
